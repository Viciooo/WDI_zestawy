...
#zwrócic wartosc 1 gdy LA > LG, wartosc -1 gdy LA < LG oraz 0 gdy LA = LG.
    ...
    for i in range(1,N-1):
        ...
        r = Sh(SubtrFract(t[i],t[i-1]))

        j = i
        while j+1 < N and Sh(MultFract(t[j],q))==t[j+1]:
            PrintFract(t[j])
            PrintFract(t[j+1])
            cg += 1
            j += 1
        ...
        j = i
        while j+1 < N and Sh(AddFract(t[j],r))==t[j+1]: #sprawdzanie LA
            PrintFract(t[j])
            PrintFract(t[j+1])
            ca += 1
            j += 1
...
def cSqrt(t):
    znak = t[1]//abs(t[1])
    return ((((t[0]+(t[0]**2+t[1]**2)**0.5)/2)**0.5),znak*(((((-1)*t[0]+(t[0]**2+t[1]**2)**0.5)/2)**0.5)))
